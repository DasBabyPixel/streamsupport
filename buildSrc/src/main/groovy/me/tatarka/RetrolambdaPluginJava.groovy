...
@CompileStatic
class RetrolambdaPluginJava implements Plugin<Project> {
    @Override
    void apply(Project project) {
        project.afterEvaluate {
            ...
            javaPlugin.sourceSets.configureEach { SourceSet set ->
                if (retrolambda.isIncluded(set.name)) {
                    ...
                    def compileJavaTask = project.tasks.named(set.compileJavaTaskName, JavaCompile).get()
                    compileJavaTask.destinationDirectory.set newOutputDir
                    ...
                    def retrolambdaTask = project.task(taskName, dependsOn: compileJavaTask, type: RetrolambdaTask) { Task task ->
                        ...
                        t.inputDir.set(newOutputDir)
                        t.outputDir.set(oldOutputDir)
